...
const Library = ({ books, onSelectBook, onLogout, user, readCounts, mandatoryBookId, onSetMandatoryBook }) => {
    ...
    // Calculate total daily reads across all books
    const totalDailyReads = Object.values(readCounts || {}).reduce((acc, curr) => acc + (curr.today || 0), 0);
    ...
    return (
        <div className="p-5 sm:p-8 min-h-screen">
            <header className="text-center mb-12 animate-fadeInDown relative">
                {/* User Section */}
                <div className="flex justify-center sm:justify-start sm:absolute sm:top-0 sm:left-0 w-full sm:w-auto mb-6 sm:mb-0 z-10">
                ...
                {blockingBook && (
                    <div className="mt-6 bg-yellow-100/90 backdrop-blur-sm text-yellow-800 px-6 py-3 rounded-2xl inline-flex items-center gap-3 shadow-lg animate-bounce-subtle mx-auto max-w-2xl">
                        ...
                        <div className="text-right">
                            ...
                            <p>יש לקרוא את הספר "{blockingBook.title}" עוד {REQUIRED_READS - (readCounts[blockingBook.id]?.total || 0)} פעמים כדי לפתוח את שאר הספרים.</p>
                        </div>
                    </div>
                )}
            </header>
            ...
            {user && (
                <DailyPuzzle dailyReads={totalDailyReads} />
            )}
